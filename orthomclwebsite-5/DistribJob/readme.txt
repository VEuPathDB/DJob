...
3. Create or choose a bin/ directory where the executables will reside.  This 
   ...
   hand-edited files. You can place it in your home or a public place:
     % mkdir $HOME/bin
...
5. Create or choose a lib/perl/ directory where the perl libraries will 
...
If you have access to CBIL's cvs repository:
   ...
   1. checkout the DistribJob install script
     % cd $COMPONENTS_HOME
     % cvs co DistribJob/install

   2. Run the install script (example below assumes you placed bin/ and lib/
      in $HOME).
     % cd $COMPONENTS_HOME
     % DistribJob/install $HOME/bin $HOME/lib/perl $HOME/test -checkoutfirst
...
If you are using a download package:
   ...
   1. Run the install script (example below assumes you placed bin/ and lib/
     ...
     % DistribJob/install $HOME/bin $HOME/lib/perl $HOME/test
